...
class Logger():
    ...
    def file_handler(self, log_path ,level="DEBUG"):
        # 创建文件的日志处理器
    ...
    def get_log(self,log_path = './log.txt'):
        # 日志器中添加控制台处理器
        self.logger.addHandler(self.console_handler())
        # 日志器中添加文件处理器
        self.logger.addHandler(self.file_handler(log_path = log_path ))
        # 返回日志实例对象
        return self.logger
...
if __name__ == "__main__" :
    ...
    logger = Logger()
    logger = logger.get_log(log_path='2.log')
    logger.info("111")
    logger.error("333")
